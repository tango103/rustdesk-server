...
pub struct RendezvousServer {
    ...
    inner: Arc<Inner>,
    outbound_whitelist: Arc<HashSet<String>>,
}
...
impl RendezvousServer {
    fn load_outbound_whitelist(path: &str) -> HashSet<String> {
        ...
        match fs::read_to_string(path) {
            Ok(content) => {
                ...
                log::info!("Loaded {} outbound whitelist IDs from {}", ids.len(), path);
            }
            Err(err) => {
                log::warn!(
                    "Failed to load outbound whitelist from {}: {}. No IDs will be allowed to initiate outbound connections.",
                    path,
                    err
                );
            }
    ...
    async fn handle_punch_hole_request(
        ...
        let mut ph = ph;
        let src_id = self.pm.get_id_by_socket_addr(addr).await;
        if src_id
            .as_ref()
            .map(|id| !self.outbound_whitelist.contains(id))
            .unwrap_or(true)
        {
            log::warn!(
                "Reject outbound punch-hole request from {} (source ID: {:?}) to {}: not in whitelist",
                addr,
                src_id,
                ph.id
            );
