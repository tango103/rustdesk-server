...
impl RendezvousServer {
        ...
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(bytes) {
            match msg_in.union {
                ...
                Some(rendezvous_message::Union::RegisterPk(rk)) => {
                    ...
                    let ip_for_map = addr.ip().to_string();
                    self.pm.update_ip_mapping(&ip_for_map, &rk.id).await;
                    let mut msg_out = RendezvousMessage::new();
    ...
    async fn update_addr(
        ...
        self.pm.update_ip_mapping(&ip, &id).await;
        let mut msg_out = RendezvousMessage::new();
    ...
    async fn handle_punch_hole_request(
        ...
        if !whitelist::is_empty() {
            let requester_ip = try_into_v4(addr).ip().to_string();
            if let Some(requester_id) = self.pm.get_id_by_ip(&requester_ip).await {
                if !whitelist::contains(&requester_id) {
                    log::warn!("Connection rejected: ID {} (IP: {}) not in whitelist", requester_id, requester_ip);
                    let mut msg_out = RendezvousMessage::new();
                    msg_out.set_punch_hole_response(PunchHoleResponse {
                ...
                log::warn!("Connection rejected: unknown requester IP {}", requester_ip);
                let mut msg_out = RendezvousMessage::new();
                msg_out.set_punch_hole_response(PunchHoleResponse {
                    failure: punch_hole_response::Failure::OFFLINE.into(),
                    ..Default::default()
