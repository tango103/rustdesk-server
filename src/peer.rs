...
type IpToIdMap = HashMap<String, String>;
lazy_static! {
    pub(crate) static ref IP_TO_ID: Mutex<IpToIdMap> = Default::default();
...
impl PeerMap {
    ...
    #[inline]
    pub(crate) async fn update_ip_mapping(&self, ip: &str, id: &str) {
        let mut map = IP_TO_ID.lock().await;
        map.insert(ip.to_string(), id.to_string());
    }

    #[inline]
    pub(crate) async fn get_id_by_ip(&self, ip: &str) -> Option<String> {
        let map = IP_TO_ID.lock().await;
        map.get(ip).cloned()
    }
}
